...
public class CalTest {
	...
	//계산용 변수 선언
	int first,second;		// first = 초기 입력하는 숫자 , second = 
	String op;		// op = 연산자를 눌렀을 때, 뒤이어 계산할 숫자를 부른다.
		...
		for(int i = 0 ; i<bOP.length;i++) {
			bOP[i].addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					tf.setText("");
					JOptionPane.showMessageDialog(null,"버튼 "+op+" 클릭");	// 계산할 연산자를 눌렀음을 안내
				}// end of actionPerformed
			});// end of ActionListener
		}// end of for
		...
		bEqual.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				second = Integer.parseInt(tf.getText());
										// 나눗셈의 정확한 값을 얻기 위해 result를 double 타입으로 지정
				switch(op) {
